...
function start_target() {
  ...
  ${jdk_dir}/bin/java  -cp log4j-core-2.12.1.jar:log4j-api-2.12.1.jar:. -Dlog4j2.configurationFile=${ROOT_DIR}/src/test/resources/log4j2.properties $* Vuln > /tmp/vuln.log 2>&1 &
  popd > /dev/null
...
function start_static_target() {
  ...
  pushd "${ROOT_DIR}/test" > /dev/null
  ${jdk_dir}/bin/java  -cp log4j-core-2.12.1.jar:log4j-api-2.12.1.jar:. -Dlog4j2.configurationFile=${ROOT_DIR}/src/test/resources/log4j2.properties -javaagent:${agent_jar} Vuln > /tmp/vuln.log 2>&1 &
  popd > /dev/null
}
...
function static_agent_configure_verbose() {
  ...
  local agent_jar=$2
  pushd "${ROOT_DIR}/test" > /dev/null
  VERBOSE_TEST_OUTPUT=$(${jdk_dir}/bin/java $PROP_VALUE -javaagent:${agent_jar}$ARG_VALUE Empty 2>&1 || true)
  popd > /dev/null
}
